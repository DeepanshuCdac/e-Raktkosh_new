...
public class NotificationRepository {
	...
	private final RowMapper<NotificationDTO> notificationRowMapper = (rs, rowNum) -> new NotificationDTO(
			rs.getLong("id"), rs.getString("title"), rs.getInt("isUrl"), rs.getString("docUrl"),
	...
	public Optional<NotificationDataDTO> getNotificationPopupById(Long id) {
		...
		return jdbcTemplate.query(sql, new Object[] { id }, rs -> {
			if (rs.next()) {
				return Optional.of(new NotificationDataDTO(id, rs.getString("hgstr_notification_title"),
						rs.getString("hgstr_notification_data"), rs.getDate("startDate")));
			}
